...
func (uc *UserCreate) AddLoginMethods(u ...*UserLoginMethod) *UserCreate {
	...
	return uc.AddLoginMethodIDs(ids...)
}
...
func (uc *UserCreate) SetDefaultMerchant(m *Merchant) *UserCreate {
	return uc.SetDefaultMerchantID(m.ID)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.LoginMethodsIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.IntroducerIDs(); len(nodes) > 0 {
		...
		_node.user_introducer = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := uc.mutation.DefaultMerchantIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
		...
		_node.user_default_merchant = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}
